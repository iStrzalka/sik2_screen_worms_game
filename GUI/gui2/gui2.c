...
static gint destroy_window(GtkWidget *widget, GdkEvent *event, gpointer data);
static gboolean expose_event(GtkWidget *widget, GdkEventExpose *event, gpointer data);
static gboolean idle_callback(gpointer data);
static void init_colors(void);
static gint keyboard_event(GtkWidget *widget, GdkEventKey *event, gpointer data);
...
void init_colors() {
  char *colors[] = {"red",  "green",   "blue",  "yellow", "brown",
                    "cyan", "magenta", "black", "violet", NULL};
  GdkColor color;
...
void draw_brush(GtkWidget *widget, gdouble x, gdouble y, char *player) {
  ...
  if (index >= 0) {
    ...
    gtk_widget_queue_draw_area(widget, (int)(x - 1.0), (int)(y - 1.0), 3, 3);
  }
}
...
gint main(gint argc, gchar *argv[]) {
  ...
  unsigned short port = 20211; // default
  ...
  g_signal_connect(window, "delete-event", G_CALLBACK(destroy_window), NULL);

  // Maska zdarzeń dla klawiatury
  gtk_widget_set_events(window, GDK_KEY_PRESS_MASK | GDK_KEY_RELEASE_MASK);

  // Callbacki dla klawiatury
  g_signal_connect(window, "key-press-event", G_CALLBACK(keyboard_event), "press");
  g_signal_connect(window, "key-release-event", G_CALLBACK(keyboard_event), "release");
  ...
  g_signal_connect(button, "pressed", G_CALLBACK(arrow_pressed), (gpointer) "left");
  g_signal_connect(button, "released", G_CALLBACK(arrow_released), (gpointer) "left");

  // Przycisk strzałki w prawo
  button = create_arrow_button(GTK_ARROW_RIGHT, GTK_SHADOW_ETCHED_OUT);
  gtk_box_pack_start(GTK_BOX(box2), button, FALSE, FALSE, 3);
  g_signal_connect(button, "pressed", G_CALLBACK(arrow_pressed), (gpointer) "right");
  g_signal_connect(button, "released", G_CALLBACK(arrow_released), (gpointer) "right");

  // Przycisk do czyszczenia ekranu (nie używany)
  button = gtk_button_new_with_label("Clear");
  gtk_box_pack_end(GTK_BOX(box2), button, FALSE, FALSE, 3);
  g_signal_connect(button, "clicked", G_CALLBACK(area_clear), (gpointer)drawing_area);
